...
@test "test1.4" {
  $RUN_TEST acclint -Wno-surface-not-convex --quiet test1.ac
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
...
@test "test1.5" {
  $RUN_TEST acclint -Wno-surface-not-convex --summary test1.ac
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
...
@test "test1.6" {
  $RUN_TEST acclint -Wno-surface-not-convex --quiet --summary test1.ac
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
