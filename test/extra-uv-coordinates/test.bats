...
@test "test1.3" {
  $RUN_TEST acclint -Wno-warnings -Wextra-uv-coordinates test1.acc
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test1.result)" ]
}
...
@test "test2.3" {
  $RUN_TEST acclint -Wno-warnings -Wextra-uv-coordinates test2.acc
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test2.result)" ]
}
...
@test "test3.3" {
  $RUN_TEST acclint -Wno-warnings -Wextra-uv-coordinates test3.acc
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test3.result)" ]
}
