...
@test "test1.3" {
  run acclint -Wno-warnings -Wduplicate-surfaces-winding test1.ac
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test1.result)" ]
}
...
@test "test2.3" {
  run acclint -Wno-warnings -Wduplicate-surfaces-winding test2.ac
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test2.result)" ]
}
...
@test "test3.3" {
  run acclint -Wno-warnings -Wduplicate-surfaces-winding test3.ac
  [ "$status" -eq 0 ]
  [ "$output" = "$(cat test3.result)" ]
}
