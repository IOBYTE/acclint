...
@test "test1.5" {
  $RUN_TEST acclint --quiet -Wsurface-strip-size test1.acc
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
...
@test "test1.6" {
  $RUN_TEST acclint --summary -Wsurface-strip-size test1.acc
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
...
@test "test1.7" {
  $RUN_TEST acclint --quiet --summary -Wsurface-strip-size test1.acc
  [ "$status" -eq 0 ]
  actual="$(echo "$output" | tr -d '\r')"
